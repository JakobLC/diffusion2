...
class DiffusionSampler(object):
    ...
    def sample(self,**kwargs):
        ...
        with torch.no_grad():
            for batch_ite in progress_bar:
                batch, model_kwargs, batch_queue = self.form_next_batch()
                x_true,model_kwargs,info = self.get_kwargs(batch)
                x_init = torch.nn.randn_like(x_true)
                ...
                for i in range(x_pred.shape[0]):
                    ...
                    if batch_queue[i]["vote"]==self.opts.num_votes-1:
                        metrics = self.run_on_full_votes(votes,x_init[i],x_true[i],info[i])
                        votes = []
        ...
        return metric_list, mean_metrics, samples
    
    def run_on_single_batch(self):
        pass

    def run_on_full_votes(self,votes,x_init,x_true,info):
        self.samples.append(votes)
        metrics = get_metrics(votes,x_true)
        return metrics
    
    def run_on_finished(self):
        pass
    ...
    def form_next_batch(self):
        ...
        for i in range(bs):
            ...
            for k in self.source_batch.keys():
