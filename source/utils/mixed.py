...
def ambiguous_info_from_fn(fn):
    """Extracts info from a fn string like the following:
    "mask-i_0/4,slice-i_0/6,nodule-i_0/4,scan_0,img_000000.dcm,mask_000000.png"
    would return
    fn_dict = {"m_i": 0, "m_tot": 4, 
               "s_i": 0, "s_tot": 6, 
               "n_i": 0, "n_tot": 4, 
               "scan_id": 0, 
               "image_id": 0, 
               "mask_id": 0}
    """
    keys = ["m_i","m_tot","s_i","s_tot","n_i","n_tot","scan_id","image_id","mask_id","rawslice"]
    i_keys = ["mask","slice","nodule"]
    fn_dict = {k: None for k in keys}
    for substring in fn.split(","):
        if any([substring.startswith(k+"-i_") for k in i_keys]):
            index_letter = substring[0]
            fn_dict[index_letter+"_i"] = int(substring.split("_")[1].split("/")[0])
            fn_dict[index_letter+"_tot"] = int(substring.split("_")[1].split("/")[1])
        elif substring.startswith("scan_"):
            fn_dict["scan_id"] = int(substring.split("_")[1])
        elif substring.startswith("rawslice_"):
            fn_dict["rawslice"] = int(substring.split("_")[1])
        elif substring.startswith("img_"):
            fn_dict["image_id"] = int(substring.split("_")[1].split(".")[0])
        elif substring.startswith("mask_"):
            fn_dict["mask_id"] = int(substring.split("_")[1].split(".")[0])
